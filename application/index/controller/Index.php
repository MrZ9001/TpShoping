...
class Index extends Controller
{
    ...
    public function showArticle($page = 1, $PageSize = 5){
        ...
        //分页处理
        $article_count = $blogArticle->count('article_id');
        ...
        $sql = $blogArticle->limit($PageStart,1)->field('article_id')->order('article_id','DESC')->select(false);
        //dump($sql);exit;
        $article_list  = $blogArticle->alias('b') //别名
                                     ->join('user u','u.user_id = b.user_id') //内连接
                                     ->where('article_id', 'exp','<=('.$sql.')')
                                     ->limit($PageSize)
